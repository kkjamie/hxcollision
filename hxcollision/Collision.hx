...
class Collision
{
    ...
    public static function testShapes(shape1:Shape, shape2:Shape):CollisionData
    {
        ...
        // Circle - Poly
        if (Std.is(shape1, Circle) && Std.is(shape2, Polygon))
        {
            return checkCircleVsPolygon(cast(shape1, Circle), cast(shape2, Polygon));
        }
        if (Std.is(shape2, Circle) && Std.is(shape1, Polygon))
        {
            return checkCircleVsPolygon(cast(shape2, Circle), cast(shape1, Polygon));
        }

		if (Std.is(shape1, ConcavePolygon)){
			cast(shape1,ConcavePolygon).transformChildren();
		}
		if (Std.is(shape2, ConcavePolygon)){
			cast(shape2,ConcavePolygon).transformChildren();
		}

		// Circle to concave poly
        if (Std.is(shape1, Circle) && Std.is(shape2, ConcavePolygon)){
            for (poly in cast(shape2,ConcavePolygon).polygons){
                var collision = checkCircleVsPolygon(cast(shape1,Circle),poly);
                if (collision != null){
                    return collision;
                }
            }
        }
        if (Std.is(shape1, ConcavePolygon) && Std.is(shape2, Circle)){
            for (poly in cast(shape1,ConcavePolygon).polygons){
                var collision = checkCircleVsPolygon(cast(shape2,Circle),poly);
                if (collision != null){
                    return collision;
                }
            }
        ...
        // Poly to concave poly
        if (Std.is(shape1, Polygon) && Std.is(shape2, ConcavePolygon)){
            for (poly in cast(shape2,ConcavePolygon).polygons){
                if (checkPolygons(cast(shape1,Polygon),poly)!=null){
					var collision = checkPolygons(poly,cast(shape1,Polygon));
					if (collision != null){return collision;}
                }
            }
        }
        if (Std.is(shape1, ConcavePolygon) && Std.is(shape2, Polygon)){
            for (poly in cast(shape1,ConcavePolygon).polygons){
				if (checkPolygons(cast(shape2,Polygon),poly) != null){
					var collision = checkPolygons(poly,cast(shape2,Polygon));
					if (collision != null){return collision;}
				}
            }
        }

        // Concave Poly - Concave Poly
        if (Std.is(shape1, ConcavePolygon) && Std.is(shape2, ConcavePolygon)){
            for (poly1 in cast(shape1,ConcavePolygon).polygons){
                for (poly2 in cast (shape2,ConcavePolygon).polygons){
                   	if (checkPolygons(poly1,poly2) != null){
						var collision = checkPolygons(poly2,poly1);
						if (collision != null){return collision;}
                    }
                }
            }
    ...
    public static function rayCollision(lineStart:Vector2D, lineEnd:Vector2D, shapes:Array<Shape>):Bool
    {
        ...
        for (_shape in shapes)
        {
            ...
            else // shape is concave polygon
            {
                //if it's not a circle, it's a polygon
                var line:Array<Vector2D> = bresenhamLine(lineStart, lineEnd);

                for (poly in cast (_shape,ConcavePolygon).polygons)
                {
                    for (_point in line)
                    {
                        if (pointInPoly(_point, poly))
                        {
                            return true;
                        } //if the point is inside the polygon
                    } //for each point in the line
    ...
    public static function bresenhamLine(start:Vector2D, end:Vector2D):Array<Vector2D>
    {
        ...
        for (x in Std.int(start.x) ... Std.int(end.x))
        { //for each point
            if (steep)
            {
    ...
    private static function checkCircleVsPolygon(circle:Circle, polygon:Polygon):CollisionData
    {
        ...
        { //project all its points, starting with the first(the 0th was done up there^)
